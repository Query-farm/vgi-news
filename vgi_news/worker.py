...
"""VGI worker exposing the ``news`` catalog (global news search) to DuckDB.

Assembles the table functions in :mod:`vgi_news` into a single ``news`` catalog
and hosts them as a worker. DuckDB spawns this over stdio and ATTACHes it::

    INSTALL vgi FROM community; LOAD vgi;
    ATTACH 'news' (TYPE vgi, LOCATION 'vgi-news-worker');
...
This module is the wheel-importable home of the worker; the repo-root
``news_worker.py`` is a thin PEP 723 shim that re-exports :class:`NewsWorker` and
:func:`main` so ``uv run news_worker.py`` keeps working for local dev, the
Makefile, and CI.
"""
...
def main() -> None:
    """Run the news worker process (stdio or, via flags, HTTP)."""
    NewsWorker.main()
